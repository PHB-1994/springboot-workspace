...
public class ProductController {
     ...
     */
    @GetMapping("/all")
    public ResponseEntity<List<Product>> getAllById(int id){
     ...
     * PathVariable = {} 형태로 {} 내부에 변수명에 해당하는 데이터로 접근
     * RequestParam = ?category="카테고리명칭" 과 같은 형태로 키:값 데이터로 접근
     * @param keyword 키워드에 해당하는 데이터를 DB 에 조회 후
     ...
     * @param product   수정할 제품에 대하여 작성된 내용 모두 가져오기
     * @return          수정된 결과를 클라이언트한테 전달
     */
    @PutMapping("/{id}")
    ...
    public ResponseEntity<Map<String, Object>> deleteProduct(@PathVariable int id){
        ...
        try {
            ...
            log.warn("상품 삭제 실패 - 유효성 검사 오류 : {}", e.getMessage());
            res.put("success", false);
            res.put("message", e.getMessage());
            return ResponseEntity.badRequest().body(res);
    ...
    public ResponseEntity<Map<String, Object>> updateStock(@PathVariable int id, @RequestParam int quantity){
        log.info("PATCH /api/product/{}/stock?quantity={} - 재고 삭제", id, quantity);
        Map<String, Object> res = new HashMap<>();
        ...
        try {
            ...
            log.warn("재고 업데이트 실패 - 유효성 검사 오류 : {}", e.getMessage());
            res.put("success", false);
            res.put("message", e.getMessage());
            return ResponseEntity.badRequest().body(res);
